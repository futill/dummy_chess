...
class ArmMoveWithInput(Node):
    def __init__(self):
        ...
        self.commands = {
            ...
            "1": ([0.345, 0.009, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "2": ([0.345, 0.045, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "3": ([0.345, 0.08, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "4": ([0.375, 0.009, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "5": ([0.375, 0.045, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "6": ([0.375, 0.08, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "7": ([0.405, 0.009, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "8": ([0.405, 0.045, 0.071], [1.0, 0.0, 0.0, 0.0]),
            "9": ([0.405, 0.08, 0.071], [1.0, 0.0, 0.0, 0.0])
        }
    ...
    def send_command(self, command_key):
        if command_key in self.commands:
            ...
            if reached:
                self.get_logger().info("✅ 机械臂末端已到达目标位置（通过 /tf 验证）")
            else:
                self.get_logger().warn("⚠️ 机械臂末端未在限定时间内到达目标位置")
